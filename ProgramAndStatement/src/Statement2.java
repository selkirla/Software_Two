...
public class Statement2 extends StatementSecondary {
    ...
    public final void addToBlock(int pos, Statement s) {
        ...
        Statement2 statementToAdd = (Statement2) s;
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();
        StatementLabel label = this.rep.disassemble(children);

        //add statement to specified position in block
        children.add(pos, statementToAdd.rep);

        //reassemble block with updated children
        this.rep.assemble(label, children);

        //create new rep for added statement
        statementToAdd.createNewRep();
    }
    ...
    public final Statement removeFromBlock(int pos) {
        ...
        //create sequence of children statements & disassemble block
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();
        this.rep.disassemble(children);

        //remove statement at specified position
        Tree<StatementLabel> removed = children.remove(pos);

        //create new label for block & reassemble it without removed statement
        StatementLabel label = new StatementLabel(Kind.BLOCK);
        this.rep.assemble(label, children);

        //create new Statement2 instance with removed statement & return it
        Statement2 removedStatement = this.newInstance();
        removedStatement.rep = removed;
    ...
    public final void assembleIfElse(Condition c, Statement s1, Statement s2) {
        ...
        //assemble if-else statement with condition & two block statements
        this.rep.assemble(label, children);

        //create new reps for statements
    ...
    public final Condition disassembleIfElse(Statement s1, Statement s2) {
        ...
        //disassemble if-else statement & get condition
        StatementLabel label = this.rep.disassemble(children);

        localS1.rep = children.remove(0);
        localS2.rep = children.remove(0);

        //create new rep for statement
        this.createNewRep();
    ...
    public final void assembleWhile(Condition c, Statement s) {
        ...
        //assemble while statement with condition & block statement
        this.rep.assemble(label, children);
    ...
    public final Condition disassembleWhile(Statement s) {
        ...
        //disassemble while statement & get condition
        StatementLabel label = this.rep.disassemble(children);
    ...
    public final void assembleCall(String inst) {
        ...
        //assemble call with instruction
    ...
    public final String disassembleCall() {
        ...
        //disassemble call & get instruction
