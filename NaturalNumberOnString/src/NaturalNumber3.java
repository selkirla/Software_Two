...
public class NaturalNumber3 extends NaturalNumberSecondary {
    ...
    public final void multiplyBy10(int k) {
        ...
        //check if rep satisfies conditions (is not empty & k is greater than 0)
        if (this.rep.length() > 0 && k > 0) {
            //convert k to string & concatenate it to existing rep
    ...
    public final int divideBy10() {
        ...
        //check if rep satisfies condition (is not empty)
        if (!this.rep.isEmpty()) {
            //get last char (digit)
            k = this.rep.substring(this.rep.length() - 1);
            //subtract last char
            this.rep = this.rep.substring(0, this.rep.length() - 1);
        }
        //convert removed char back to int & return as remainder
    ...
    public final boolean isZero() {
        ...
        //check if rep is empty to see if its 0, if so change result
        if (this.rep.isEmpty()) {
            result = true;
        }
